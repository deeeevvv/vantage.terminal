...
const currencies = {
    ...
    "KES": {n: "Kenyan Shilling", c: "ke"}, "KWD": {n: "Kuwaiti Dinar", b: "kw"}, "LBP": {n: "Lebanese Pound", c: "lb"},
...
async function updateRates() {
    try {
        ...
        let colorVar = percentage < 5 ? "var(--cost-low)" : (percentage < 15 ? "var(--cost-med)" : "var(--cost-high)");
    ...
    } catch (e) { console.error("Update rates failed"); }
}
...
/* -------------------------------------------------------------------------- */
/* MACRO INTELLIGENCE (NEWS) - FIXED VERSION                                  */
/* -------------------------------------------------------------------------- */
...
async function fetchNews() {
    ...
    // We use individual try/catch for each source so one failure doesn't stop the feed.
    const sources = [
        "https://finance.yahoo.com/news/rssindex",
        "https://finance.yahoo.com/rss/currencies",
        "https://news.google.com/rss/search?q=forex+market+news&hl=en-IN&gl=IN&ceid=IN:en"
    ];
    ...
    try {
        ...
        const uniqueItems = Array.from(new Map(combinedItems.map(item => [item.title, item])).values());
        ...
        console.error("Critical News Feed Error:", e);
    }
...
function displayNews() {
    ...
    newsFeed.innerHTML = slice.map(item => {
        ...
        if (!img) {
            const keywords = item.title.split(' ').slice(0, 3).join('%20');
            img = `https://image.pollinations.ai/prompt/financial%20news%20${keywords}?width=320&height=200&nologo=true`;
        }
        ...
        let source = item.author || "Market News";
    ...
    document.querySelector("#load-more-news").style.display = (displayedNewsCount >= allNewsItems.length || allNewsItems.length === 0) ? "none" : "block";
}
...
window.loadPair = (pair) => {
    ...
    updateTriggerContent(document.getElementById("from-wrapper").querySelector(".custom-select-trigger"), f);
    updateTriggerContent(document.getElementById("to-wrapper").querySelector(".custom-select-trigger"), t);
    updateRates();
};
...
document.querySelector("#swap-btn").onclick = () => {
    ...
    updateTriggerContent(document.getElementById("from-wrapper").querySelector(".custom-select-trigger"), fromCurr.value);
    updateTriggerContent(document.getElementById("to-wrapper").querySelector(".custom-select-trigger"), toCurr.value);
    updateRates();
};
